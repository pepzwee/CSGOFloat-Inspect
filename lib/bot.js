...
class Bot extends EventEmitter {
    ...
    getEmail(callback, retries = 10) {
        axios
            .get(`https://octopi.xyz/${this.username}@octopi.xyz.json`)
            .then(res => {
                const emails = res.data.sort((b, a) => new Date(a.receivedDate) - new Date(b.receivedDate));
                const code = emails[0].html.split('font-weight: bold;\"> ')[1].split(' </span>\n\t\t\t\t\t\t</div>')[0];

                winston.info(`Code for ${this.username} is ${code}`);
                callback(code);
            })
            .catch(err => {
                if (retries) {
                    this.getEmail(callback, retries - 1);
                }
                else {
                    callback('false');
                }
    ...
    bindEventHandlers() {
        this.steamClient.on('steamGuard', (domain, callback) => {
            winston.info(`${this.username} has a pending e-mail for ${domain}`);
            
            setTimeout(() => this.getEmail(code => callback(code)), 5000);
        });
